<template>
  <div class="chessGame">
    <SlideLength @select="selectSlide" />
    <button id="backChess" @click="backChess">悔棋</button>
    <div id="preview" :style="display"></div>
    <div class="container">
      <ChessRow
  ...
  watch: {
    chessData: {
      ...
      handler(newValue) {
        ...
        this.result=newValue.some((itemRow, indexRow) => {
          return  itemRow.some((itemColumn, indexColumn) => {
            if (
    ...
    display:{
      ...
      get(){
        if(this.currentColor==1){
          return "background: radial-gradient(circle,white,rgb(16, 15, 15),black);"
        }else{
